import React from 'react';

const Home = () => {
  return (
    <div className="bg-[#0b1a29] min-h-screen pt-12 pb-24 px-10 text-white font-sans">
      <div className="max-w-7xl mx-auto">
        
        {/* === HERO SECTION === */}
        <section className="grid md:grid-cols-2 gap-12 items-center mb-32">
          <div className="flex flex-col items-start">
            {/* O triângulo/logo centralizado acima do texto (opcional no mobile) */}
            <div className="mb-8 flex justify-center w-full md:hidden">
               {/* Logo placeholder */}
            </div>

            <h1 className="text-5xl md:text-6xl font-bold leading-tight mb-6">
              Maximize Sua <br /> Eficiência em RH
            </h1>
            <p className="text-xl text-[#5cc7d9] mb-10 font-light">
              O motor centralizador da sua gestão de RH.
            </p>
            <button className="bg-[#5cc7d9] text-[#0b1a29] px-8 py-3 rounded font-bold hover:bg-[#4eb3c4] transition-colors shadow-[0_0_15px_rgba(92,199,217,0.3)]">
              Solicitar Demonstração
            </button>
          </div>

          {/* Ilustração Visual (Mockup Dashboard) */}
          <div className="relative h-96 bg-[#10263a] rounded-2xl border border-[#1a3045] shadow-2xl overflow-hidden flex items-center justify-center p-6">
            {/* Elementos decorativos simulando os gráficos da imagem */}
            <div className="absolute top-4 left-4 flex gap-2">
              <div className="h-3 w-3 rounded-full bg-gray-500/50"></div>
              <div className="h-3 w-3 rounded-full bg-gray-500/50"></div>
              <div className="h-3 w-3 rounded-full bg-gray-500/50"></div>
            </div>
            
            {/* Nó central e conexões (abstração visual) */}
            <div className="relative w-full h-full flex items-center justify-center">
               <div className="absolute w-64 h-64 border border-[#5cc7d9]/20 rounded-full animate-pulse"></div>
               <div className="absolute w-48 h-48 border border-[#5cc7d9]/40 rounded-full"></div>
               <div className="w-16 h-16 bg-[#5cc7d9] rounded-full shadow-[0_0_30px_#5cc7d9] z-10"></div>
               
               {/* Cards de painel simulados */}
               <div className="absolute top-10 right-10 bg-[#0b1a29] border border-[#1a3045] p-3 rounded-lg w-32">
                 <div className="h-2 w-full bg-[#1a3045] rounded mb-2"></div>
                 <div className="h-2 w-2/3 bg-[#5cc7d9] rounded"></div>
               </div>
               <div className="absolute bottom-10 left-10 bg-[#0b1a29] border border-[#1a3045] p-3 rounded-lg w-40 flex gap-2 items-end h-20">
                 <div className="w-4 bg-[#1a3045] h-1/2 rounded-t"></div>
                 <div className="w-4 bg-[#5cc7d9] h-full rounded-t"></div>
                 <div className="w-4 bg-[#1a3045] h-3/4 rounded-t"></div>
               </div>
            </div>
          </div>
        </section>

        {/* === RECURSOS PRINCIPAIS === */}
        <section className="mb-32">
          <h2 className="text-2xl font-bold mb-8">Recursos Principais</h2>
          <div className="grid md:grid-cols-3 gap-6">
            
            {/* Card 1 */}
            <div className="bg-[#10263a] p-8 rounded-xl border border-[#1a3045] hover:border-[#5cc7d9]/50 transition-colors">
              {/* Ícone de Folha */}
              <div className="w-12 h-12 mb-6 border border-[#1a3045] rounded bg-[#0b1a29] flex items-center justify-center text-[#5cc7d9]">
                <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
              </div>
              <h3 className="text-xl font-bold mb-4">Automação de Folha</h3>
              <p className="text-sm text-[#a3b8cc] leading-relaxed">
                Automação de folha completa e automação não conteio da estilização do folha.
              </p>
            </div>

            {/* Card 2 */}
            <div className="bg-[#10263a] p-8 rounded-xl border border-[#1a3045] hover:border-[#5cc7d9]/50 transition-colors">
              {/* Ícone de Ponto */}
              <div className="w-12 h-12 mb-6 border border-[#1a3045] rounded bg-[#0b1a29] flex items-center justify-center text-[#5cc7d9]">
                <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
              </div>
              <h3 className="text-xl font-bold mb-4">Gestão de Ponto</h3>
              <p className="text-sm text-[#a3b8cc] leading-relaxed">
                Gestão de Ponto, gestão conectando dados em um sistema de visão diária na gestão de Ponto.
              </p>
            </div>

            {/* Card 3 */}
            <div className="bg-[#10263a] p-8 rounded-xl border border-[#1a3045] hover:border-[#5cc7d9]/50 transition-colors">
              {/* Ícone de Recrutamento */}
              <div className="w-12 h-12 mb-6 border border-[#1a3045] rounded bg-[#0b1a29] flex items-center justify-center text-[#5cc7d9]">
                <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
              </div>
              <h3 className="text-xl font-bold mb-4">Recrutamento</h3>
              <p className="text-sm text-[#a3b8cc] leading-relaxed">
                Recrutamento com rastreamento, conectado com remote online e canais de recrutamento.
              </p>
            </div>

          </div>
        </section>

        {/* === DEPOIMENTOS === */}
        <section>
          <h2 className="text-2xl font-bold mb-8">Depoimentos</h2>
          
          <div className="flex items-center gap-6 w-full max-w-5xl">
            {/* Seta Esquerda */}
            <button className="w-10 h-10 rounded-full bg-[#10263a] flex items-center justify-center border border-[#1a3045] hover:bg-[#1a3045] text-[#a3b8cc] transition-colors shrink-0">
              <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"></path></svg>
            </button>

            {/* Card do Depoimento */}
            <div className="bg-[#10263a] p-8 rounded-xl border border-[#1a3045] flex flex-col md:flex-row items-center gap-8 w-full">
              {/* Imagem Placeholder parecida com a do mockup */}
              <img 
                src="https://i0.wp.com/cromossomonerd.com.br/wp-content/uploads/2017/03/destaque_homem_aranha.png?fit=1068%2C600" 
                alt="Foto de perfil" 
                className="w-32 h-32 rounded-lg object-cover shadow-lg shrink-0"
              />
              
              <div className="flex-1">
                <p className="italic text-[#a3b8cc] mb-6 text-lg leading-relaxed">
                  “Equipe muito profissional e atenciosa. A StaffCore tornou nosso processo de contratação mais rápido, simples e eficiente!”
                </p>
                <h4 className="font-bold text-white text-lg">Peter Parker</h4>
                <span className="text-sm text-[#5cc7d9]">SpiderTech</span>
              </div>
            </div>

            {/* Seta Direita */}
            <button className="w-10 h-10 rounded-full bg-[#10263a] flex items-center justify-center border border-[#1a3045] hover:bg-[#1a3045] text-[#a3b8cc] transition-colors shrink-0">
              <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"></path></svg>
            </button>
          </div>
        </section>

      </div>
    </div>
  );
};

export default Home;